...
namespace MedicForm
{
    public partial class Result : Form
    {
        ...
        private void delSeriaButt_Click(object sender, EventArgs e)
        {
            ...
            dbManager.DeleteFromDB("parameters_value", "calculation_number", number.ToString());
            this.Refresh();
            getSerias();
            showResult();
        ...
        private void delPokaz_Click(object sender, EventArgs e)
        {
            ...
            var parametersIds = dbManager.GetRows("formula_compound", "id_of_parameter", "id_of_formula = " + formula_id);
            string[] delPole = {"id_of_parameter", "calculation_number", "id_of_expert"};

            for (int i = 0; i < parametersIds.Count; i++)
            {
                string[] delValues = {parametersIds[i][0].ToString(), id.ToString(), "3"};
                dbManager.DeleteFromDB("parameters_value", delPole, delValues);
            }
