...
namespace Experts_Economist
{
    public partial class Redakt : Form
    {
        ...
        {
            for (int i = 0; i < formulasLB.Items.Count; i++)
            {
                if (formulasLB.Items[i].ToString().StartsWith(search_formula.Text))
                {
        ...
        private void search_parametr_TextChanged(object sender, EventArgs e)
        {
            for (int i = 0; i < componentLB.Items.Count; i++)
            {
                if (componentLB.Items[i].ToString().StartsWith(search_parametr.Text))
                {
                    componentLB.SelectedIndex = i;
                    break;
                }
