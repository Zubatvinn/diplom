...
namespace Experts_Economist
{
    public partial class Rozrah : Form
    {
        ...
        private void get_values()
        {
            ...
            var obj01 = db.GetRows("calculations_description group by calculation_number", "calculation_number", "");
            ...
            var obj02 = db.GetRows("calculations_description", "Max(calculation_number)", "");
            try
            {
        ...
        private void formulasLB_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                obj1 = db.GetRows("formula_parameters", "name_of_parameter, measurement_of_parameter,description_of_parameter", "id_of_parameter = " + obj[i][0].ToString());
        ...
        private void Iterations_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            for (int j = 0; j < it; j++)
            {
                var obj = db.GetRows("formula_compound", "id_of_parameter", "id_of_formula = " + idf);//переменная для хранения списка параметров привязанных к данной формуле
                var obj1 = new List<List<Object>>();//переменная для хранения имени параметра и единиц измерения
                if (Convert.ToInt32(idf) == 15 || Convert.ToInt32(idf) == 16)
                    help1 = 1;
            ...
            {
                var obj = db.GetRows("formula_compound", "id_of_parameter", "id_of_formula = " + idf);//переменная для хранения списка параметров привязанных к данной формуле
                var obj1 = db.GetRows("formula_parameters", "name_of_parameter, measurement_of_parameter,description_of_parameter", "id_of_parameter = " + obj[obj_items][0].ToString());
                this.formulasDGV.Rows.Add(obj1[0][0].ToString(), "0", obj1[0][1].ToString(), obj1[0][2].ToString());
            }
            help = true;//вспомогательная переменная для проверки наличия строки Result с результатом исчисления 
        }
        ...
        private void Save_values_Click(object sender, EventArgs e)
        {
            ...
            {
                var obj2 = db.GetRows("calculations_description", "Max(calculation_number)", "");
                if (obj2.Count == 1)
                    calc_numbCB.Text = "1";
                else
            ...
            string[] fields1 = { "calculation_number" };
            string[] values1 = { calc_numbCB.Text };
            try
            {
        ...
        private void series_over_Click(object sender, EventArgs e)
        {
            ...
            string[] fields4_1 = { "calculation_number", "calculation_name", "description_of_calculation", "issue_id" };
            string[] values4_1 = { calc_numbCB.Text, "'" + name_of_seriesTB.Text + "'", "'" + desc_of_seriesTB.Text + "'", issueid.ToString() };
            try
            {
